...
export const VIDEOS_LIST_URL = `https://youtube.googleapis.com/youtube/v3/videos?part=snippet,contentDetails,statistics&fields=items(id,contentDetails(duration),snippet(title,description,channelTitle,publishedAt,thumbnails(high(url))),statistics(viewCount))&key=${process.env.REACT_APP_API_KEY}`;
...
export const RELATED_VIDEO_IDS_URL = `https://youtube.googleapis.com/youtube/v3/search?part=snippet&fields=items(id,snippet(title))&type=video&maxResults=50&nextPageToken&key=${process.env.REACT_APP_API_KEY}`;
...
export const VIDEO_IDS_URL = `https://youtube.googleapis.com/youtube/v3/search?part=snippet&fields=items(id,snippet(title))&type=video&maxResults=50&nextPageToken&key=${process.env.REACT_APP_API_KEY}`;
